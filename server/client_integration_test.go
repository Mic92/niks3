...
func TestClientIntegration(t *testing.T) {
	...
	// Build the Rust client first
	t.Log("Building Rust client...")
	...
	err = os.WriteFile(tempFile, []byte("test content for niks3 integration test"), 0o644)
	ok(t, err)
	...
	cmd.Env = append(os.Environ(),
		fmt.Sprintf("NIKS3_SERVER_URL=%s", ts.URL),
		fmt.Sprintf("NIKS3_AUTH_TOKEN=%s", authToken),
		"RUST_LOG=info",
	)
	...
	narinfoKey := fmt.Sprintf("%s.narinfo", hash)
	...
	if !strings.Contains(narinfoStr, fmt.Sprintf("StorePath: %s", storePath)) {
...
func TestClientMultipleUploads(t *testing.T) {
	...
	for _, storePath := range storePaths {
		...
		if err != nil {
			t.Errorf("Narinfo not found in S3 for %s: %v", storePath, err)
		}
		...
		_, err = testService.MinioClient.StatObject(context.Background(), testService.Bucket, narKey, minio.StatObjectOptions{})
		if err != nil {
			t.Errorf("NAR not found in S3 for %s: %v", storePath, err)
		}
	}
}
...
func TestClientWithDependencies(t *testing.T) {
	...
	''
	`
	err = os.WriteFile(nixExpr, []byte(nixContent), 0o644)
	if err != nil {
		t.Fatalf("Failed to write nix expression: %v", err)
	}
	...
	t.Logf("Uploaded derivation with dependencies in %v", duration)
	t.Logf("Client output: %s", output)
	...
	t.Logf("Successfully verified %d uploaded objects", uploadedCount)
	...
	t.Run("RetrieveWithNixCopy", func(t *testing.T) {
		...
		testEnv := append(os.Environ(),
			...
			fmt.Sprintf("AWS_SECRET_ACCESS_KEY=%s", testMinioServer.secret),
		)
		...
		output, err := cmd.CombinedOutput()
		if err != nil {
			t.Fatalf("Failed to fetch nix-cache-info: %v\nOutput: %s", err, output)
		}
		t.Logf("Successfully fetched nix-cache-info")
		...
		output, err = cmd.CombinedOutput()
		if err != nil {
			t.Logf("Failed to get store info: %v\nOutput: %s", err, output)
		} else {
			t.Logf("Store info: %s", output)
		}
		...
		narinfoObj.Close()
		if err != nil {
			t.Fatalf("Failed to read narinfo: %v", err)
		}
		t.Logf("Narinfo content for %s:\n%s", storePath, narinfoContent)

		// Try the approach from Nix's own tests - copy directly without path-info first
		t.Logf("Attempting to copy store path: %s", storePath)
		t.Logf("From binary cache: %s", binaryCacheURL)
		...
		output, err = cmd.CombinedOutput()
		if err != nil {
			// If that fails, let's try with store info first
			t.Logf("Direct copy failed, trying with store info: %v\nOutput: %s", err, output)

			// Try path-info to debug
			cmd = exec.Command("nix", "path-info",
				"--store", binaryCacheURL,
				storePath)
			cmd.Env = testEnv

			output, err = cmd.CombinedOutput()
			if err != nil {
				t.Fatalf("Failed to get path info from binary cache: %v\nOutput: %s", err, output)
			}
		}

		t.Log("Successfully copied from binary cache")

		// Verify the path exists locally now
		cmd = exec.Command("nix", "path-info", storePath)
		output, err = cmd.CombinedOutput()
		if err != nil {
			t.Errorf("Path not found locally after copy: %v\nOutput: %s", err, output)
		} else {
			t.Logf("Path verified locally: %s", output)
		}
	})
}
