...
func runServer(opts *options) error {
	...
	if opts.MTLSProxySocket != "" {
		handler = stripHeaders(mux, opts.MTLSProxyHeader, opts.MTLSSubjectHeader)
	}

	endpoints := []endpoint{{server: newServer(opts.HTTPAddr, handler)}}

	if opts.MTLSProxySocket != "" {
		endpoints = append(endpoints, endpoint{
			server: newServer(opts.MTLSProxySocket, mux),
			name:   proxyListenerName,
		})
	}
	...
	if opts.TLSCert != "" {
		...
		endpoints[0].server.TLSConfig = tlsCfg
		endpoints[0].tls = true
...
// proxyListenerName is the FileDescriptorName= of the proxy socket.
const proxyListenerName = "proxy"
...
// stripHeaders drops headers a client could forge to pass as the proxy.
func stripHeaders(next http.Handler, names ...string) http.Handler {
...
// An address starting with "/" is a unix socket.
func bindEndpoints(endpoints []endpoint) error {
	...
	for i := range endpoints {
		...
		network := "tcp"
		if strings.HasPrefix(ep.server.Addr, "/") {
			network = "unix"
		}

		ln, err := net.Listen(network, ep.server.Addr) //nolint:noctx // listener lives for the whole server lifetime
		if err != nil {
			closeAll()
