...
const defaultLeadHeartbeat = 5 * time.Second

func (s *Service) leadHeartbeat() time.Duration {
	if s.LeadHeartbeat > 0 {
		return s.LeadHeartbeat
	}

	return defaultLeadHeartbeat
}
...
func (s *Service) LeadHandler(w http.ResponseWriter, r *http.Request) {
	...
	tick := time.NewTicker(s.leadHeartbeat())
	defer tick.Stop()
